...
async function proxyMzut<T = Record<string, unknown>>(fn: string, params: Record<string, string>): Promise<T | null> {
...
export async function login(loginValue: string, password: string): Promise<SessionData> {
  ...
  if (!login || !cleanPassword) {
    throw new Error('Wpisz login i haslo.');
  }
  ...
  if (status !== 'OK') {
    if (status === 'SYSTEM ERROR') throw new Error('Blad systemu mZUT.');
    throw new Error('Niepoprawny login lub haslo.');
  }
...
function eventTypeClass(event: Record<string, string>): string {
  ...
  if (hay.includes('wyklad') || hay.includes('wykĹ‚ad') || short === 'w') return 'lecture';
...
function eventTypeLabel(typeClass: string, event: Record<string, string>): string {
  ...
  return labels[typeClass] || event.lessonForm || 'Zajecia';
}
...
export async function fetchPlan(
  ...
  if (viewMode !== 'month') {
    for (let day = new Date(rangeStart); day <= rangeEnd; day = addDays(day, 1)) {
      ...
      const dayEvents = (grouped.get(key) ?? []).map((event) => {
